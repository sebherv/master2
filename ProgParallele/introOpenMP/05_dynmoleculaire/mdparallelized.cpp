...
// Loop over time:
    ...
    while (t < t_max) {
        ...
        #pragma omp parallel shared(ke,pe)
        {
            double private_ke = 0.0;
            double private_pe = 0.0;
    ...
    // Compute forces acting on each particle and their energies
            ...
            for (int p=0; p<numParticles; p++) {
                ...
                private_ke += particle[p].getKineticEnergy();
                private_pe += particle[p].getPotentialEnergy();
            } // for p
